...
using StatPtr = shared_ptr<Statement>;
...
public:
    Token kind;
    Vector<shared_ptr<VariableDeclarator>> declarations = {};
...
{
public:
    Vector<shared_ptr<Statement>> src;
    ArrayInit(){};
    StatementType Type()
    {
...
TokenIter CalcEndBrackets(TokenFlowView tfv)
{
    ...
    vector<KW> parenthesis_stack{s_bracket};
...
{
    template <class T>
    using WithEnd = tuple<T, TokenIter>;
...
public:
    StatPtr CreateLiteralNode(const Token &tok)
    {
        ...
        {
            return make_shared<NumberLiteralInit>(tok);
        }
        else if (tok.IsBoolLiteral())
        {
            return make_shared<BoolLiteralInit>(tok);
        }
        else if (tok.IsStringLiteral())
        {
            return make_shared<StringLiteralInit>(tok);
        }
        THROW
    }
    ...
    WithEnd<shared_ptr<VariableDeclaration>> ResolveDeclearStatment(TokenFlowView tfv)
    {
        auto iter = tfv.BeginIter();
        auto var_declear = make_shared<VariableDeclaration>(*iter);
        auto id_iter = iter + 1;
        auto decl = make_shared<VariableDeclarator>(Identifier(*id_iter));
        decl->type = *iter;
    ...
    StatPtrWithEnd ResolveSubNumberLiteral(StatPtr pos1_stat, TokenFlowView tfv)
    {
        ...
        auto stat = make_shared<BinaryExpression>();
        stat->left = pos1_stat;
    ...
    StatPtrWithEnd ResolveIndex(StatPtr pos1_stat, TokenFlowView tfv)
    {
        ...
        iter++;
        auto [pos2_stat, pos2_end_iter] = ResolveExecutableStatment(iter);
        auto stat = make_shared<IndexStatement>();
        stat->object = static_pointer_cast<Identifier>(pos1_stat);
    ...
    StatPtrWithEnd ResolveExpr(StatPtr pos1_stat, TokenFlowView tfv)
    {
        ...
        {
            auto stat = make_shared<ConditionExpression>();
            stat->cond = pos1_stat;
            stat->left = pos2_stat;
        ...
        if (pos2_stat->Type() == StatementType::binaryExpression) // 调整运算的优先级
        {
            ...
            {
                auto stat_adj = make_shared<BinaryExpression>();
    ...
    {
        auto iter = tfv.BeginIter();
        auto stat = make_shared<FunctionDeclaration>();
        stat->start = *iter;
        iter++;
        while (true)
        {
    ...
    {
        auto iter = tfv.BeginIter(); // (
        auto stat = make_shared<FunctionCall>();
        stat->id = func;
        auto end_iter = CalcEndBrackets(iter); // )
        iter++;
        while (iter != end_iter)
    ...
    StatPtrWithEnd ResolveArrayInit(TokenFlowView tfv)
    {
        ...
        iter++;
        auto arr = make_shared<ArrayInit>();
    ...
    StatPtrWithEnd ResolveExecutableStatment(TokenFlowView tfv)
    {
        ...
        {
            auto left_stat = make_shared<Identifier>(*iter);
            return SeekIfExpr(left_stat, iter);
        }
        THROW_TOKEN(*iter)
    }
    ...
    StatPtrWithEnd ResovleAssigmentOrIdentify(TokenFlowView tfv)
    {
        ...
        if (assigment_or_other->Is(assigment_))
        {
            ...
            auto assigment = make_shared<AssigmentStatement>();
            assigment->id = *iter;

...
public:
    ...
    {
        std::cout << ToMs(scale) << " ms" << endl;
    }
    double ToMs(double scale = 1)
    {
        return std::chrono::duration_cast<chrono::milliseconds>(r).count() / scale;
    }
    chrono::nanoseconds Reduce()
    {
        return r;
    }
    void Reset()
...
public:
    ...
    String toStringContent(bool ignore_check = false) const
    {
        ...
        throw logic_error(String::Format("仅允许字符串字面量令牌调用 src:{}", kw));
    }

...
class Token : public ViewEnd
{
  ...
  public:
    ...
    KW kw_enum;
    Token(String _kw = "", int _pos = -1, int _line = -1, int _offset = -1, String _file = "")
        : kw(_kw), pos(_pos), line(_line), offset(_offset), file(_file)
    {
    }
    Value ToJson() const
    {
    ...
    String UniqId() { return file + ":" + String(pos); }
    ...
    bool IsLineEndToken() const { return Include({';', '\n', '\r'}); }

    bool Is(String sym) const { return kw == sym; }

    bool IsComma() const { return Is(comma_); }
    ...
    // 数字字面量
    bool IsNumericLiteral() const { return IsNumber(kw); }
    ...
    bool IsBoolLiteral() const { return Include({"true", "false"}); }

    bool IsNull() const { return kw == "null"; }

    bool IsLiteral() const { return IsStringLiteral() || IsBoolLiteral() || IsNumericLiteral() || IsNull(); }

    bool Is(KW kw) const { return Is(Kw2Str(kw)); }
    ...
    double toNumber(bool ignore_check = false) const { return ignore_check ? std::stod(kw) : StringSafe2Double(kw); }
    ...
    // 包含
    bool Include(const Vector<String>& syms) const { return syms.Includes(kw); }

    // 包含
    bool Include(const Vector<KW>& syms) const { return syms.Map<String>(Kw2Str).Includes(kw); }
    ...
    static KW Str2kw(String kw, bool throw_if_not_found = true)
    {
...
void ThrowLogicError(String msg) { throw std::logic_error(msg); }
...
class GeneralTokenizer
{
  ...
  public:
    ...
    static Vector<Token> Agumi(const String& script, String file = GeneralTokenizer::ReplFileName())
    {
        ...
        return GeneralTokenizer(script, GeneralTokenizer::agumi, file).Start().Filter([](Token tok) { return tok.kw != " "; });
    }

    static String ReplFileName() { return String::Format("repl_{}", ++GeneralTokenizer::uniq_id); }

  private:
    Token CreateToken(String src) { return Token(src, ptr, line, offset, file_name); }
    String file_name;
    ...
    bool NotLast() { return ptr < src.size() - 1; }
    char Next() { return src[ptr + 1]; }
    ...
    void Identify(char c)
    {
        ...
        {
            char c_next = src[ptr_next];
            if (!(('a' <= c_next && c_next <= 'z') || ('A' <= c_next && c_next <= 'Z') || ('0' <= c_next && c_next <= '9') ||
                  c_next == '_' || c_next == '$'))
            {
                break;
            }
    ...
    static bool CompareStrLen(String s1, String s2) { return s1.length() > s2.length(); }

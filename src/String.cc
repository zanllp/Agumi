...
String::String(bool arg) { (*this) = arg ? "true" : "false"; }
...
Vector<String> String::USpilt() const
{
    ...
    for (auto v = utf8codepoint(data, &codepoint); '\0' != codepoint; v = utf8codepoint(v, &codepoint))
    {
        memset(str, 0, 5);
...
size_t String::Ulength() const { return utf8len(data()); }
String String::USubStr(int start, int count) const
{
    ...
    int codepoint;
    int code_n = 0;
    Assest(start >= 0 && (count >= 0 || count == -1), String::Format("start:{}, count:{} ", start, count));
    for (auto v = utf8codepoint(data, &codepoint); code_n != start; v = utf8codepoint(v, &codepoint))
    {
        auto size = utf8codepointsize(codepoint);
        if (v != 0)
        {
    ...
    size_t need_len = 0;
    code_n = 0;
    for (auto v = utf8codepoint(data, &codepoint); code_n < count; v = utf8codepoint(v, &codepoint))
    {
        auto size = utf8codepointsize(codepoint);
        if (v != 0)
        {
...
String String::Unescape() const { return Split("\\\"", -1, false).Join("\""); }

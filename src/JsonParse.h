...
{
    // tfv
    static void ResolveObject(const TokenFlowView& tfv, Object& obj)
    {
    ...
    static void ResolveArray(const TokenFlowView& tfv, Array& arr)
    {
        auto iter = tfv.BeginIter();
        iter++;
        if (tfv.IsEnd(iter))
    ...
    static std::tuple<Value, TokenIter> ResolveValue(const TokenFlowView& tfv)
    {
        Value res;
  ...
  public:
    static Value Call(const String& src)
    {
        Value res;
